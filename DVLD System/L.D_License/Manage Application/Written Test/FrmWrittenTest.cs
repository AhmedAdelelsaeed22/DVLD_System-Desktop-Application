...
{
    public partial class FrmWrittenTest : MaterialSkin.Controls.MaterialForm
    {
        ...
        private void DataGridViewHandlerWidth()
        {
            ...
            DataTable dtSourceTestAppointments = _AppointmentInfo.GetAllWrittenTestAppointments
                    (_LocalDrivingLicenseApplicationID);
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            if (dgvTestAppointments.Rows.Count != 0)
            {
                ...
                {
                    FrmAddWrittenTest frmAddNewWrittenTest
                    = new FrmAddWrittenTest(2, _LDAppInfo, _ApplicationInfo);
                    frmAddNewWrittenTest.ShowDialog();
                    ReloadData();
                    RecordsNumber();
                }
            }
            ...
            {
                FrmAddWrittenTest frmAddNewWrittenTest
                = new FrmAddWrittenTest(_LDAppInfo, _ApplicationInfo);
                frmAddNewWrittenTest.ShowDialog();
                ReloadData();
                RecordsNumber();
            }
        ...
        private void takeTestToolStripMenuItem_Click(object sender, EventArgs e)
        {
            int AppointmentID = Convert.ToInt32
                (dgvTestAppointments.CurrentRow.Cells[0].Value);
            FrmWrittenTakeTest frmWrittenTakeTest = new FrmWrittenTakeTest
                (AppointmentID, _LDAppInfo, _ApplicationInfo);
            frmWrittenTakeTest.ShowDialog();
            ReloadData();
        }
    }
}
