...
{
    public partial class FrmStreetTest : MaterialSkin.Controls.MaterialForm
    {
        ...
        private void DataGridViewHandlerWidth()
        {
            ...
            DataTable dtSourceTestAppointments = _AppointmentInfo.GetAllStreetTestAppointments
                    (_LocalDrivingLicenseApplicationID);
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            if (dgvTestAppointments.Rows.Count != 0)
            {
                ...
                {
                    FrmAddStreetTest frmAddNewStreetTest
                    = new FrmAddStreetTest(2, _LDAppInfo, _ApplicationInfo);
                    frmAddNewStreetTest.ShowDialog();
                    ReloadData();
                    RecordsNumber();
                }
            }
            ...
            {
                FrmAddStreetTest frmAddNewStreetTest
                = new FrmAddStreetTest(_LDAppInfo, _ApplicationInfo);
                frmAddNewStreetTest.ShowDialog();
                ReloadData();
                RecordsNumber();
            }
        ...
        private void takeTestToolStripMenuItem_Click(object sender, EventArgs e)
        {
            int AppointmentID = Convert.ToInt32
             (dgvTestAppointments.CurrentRow.Cells[0].Value);
            FrmStreetTakeTest frmStreetTakeTest = new FrmStreetTakeTest
                (AppointmentID, _LDAppInfo, _ApplicationInfo);
            frmStreetTakeTest.ShowDialog();
            ReloadData();


            if (clsTests.IsPassedTest(AppointmentID))
            {
                clsLocalDrivingLicenseApplication.UpdateApplicationStatus
                   (_ApplicationInfo.ApplicationID , 3);
            }
        }
